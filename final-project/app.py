...
def signup():
    ...
    if request.method == "POST":
        ...
        if not full_name or not email or not password:
            ...
            flash("هذا البريد الإلكتروني مسجل بالفعل. جربي تسجيل الدخول.", "error")
        else:
            try:
                user_id = db.create_user(full_name, email, generate_password_hash(password))
            except Exception:
                flash("تعذّر إنشاء الحساب حالياً بسبب خطأ فني. حاولي مرة أخرى.", "error")
            else:
...
def audit():
    ...
    try:
        ...
        flash("تعذّرت قراءة ملف PDF. تأكدي أن الملف غير تالف وحاولي مرة أخرى.", "error")
        return render_template("audit.html", audit=None, supplier_name=supplier_name)
    ...
    try:
        ...
        flash("تعذّر إتمام عملية التدقيق حالياً. قد تكون هناك مشكلة في الاتصال بمحرك الذكاء الاصطناعي — حاولي مرة أخرى بعد قليل.", "error")
        return render_template("audit.html", audit=None, supplier_name=supplier_name)
